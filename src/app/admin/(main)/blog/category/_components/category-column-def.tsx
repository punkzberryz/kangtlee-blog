...
export const categoryColumnDef: ColumnDef<PostCategory>[] = [
  {
    header: ({ column }) => <TableColumnHeader title="id" column={column} />,
    accessorKey: "id",
    size: 20,
    cell: ({ row }) => (
      <Link href={`/admin/blog/category/${row.original.id}`}>
        {row.original.id}
      </Link>
    ),
  },
  ...
  },
  {
    ...
    accessorKey: "edit",
    cell: ({ row }) => (
      <ColumnEditButton href={`/admin/blog/category/${row.original.id}`} />
    ),
    size: 40,
  },
  {
